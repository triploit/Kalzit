	...
	if(!notificationAllowed()) {
		...
		withNotification(Notification => {
			...
			notification.onclick = function(event) {
				...
				GLang.call(GLang.dr.resolveName("show_message_async"), [GLang.voidValue, GLang.stringValue(text)]);
				notification.close();
			}
		})
	}
...
this.notification_show_long_text = function(summary, full){
	if(!notificationAllowed()) {
		...
		withNotification(Notification => {
			...
			notification.onclick = function(event) {
				event.preventDefault(); // prevent the browser from focusing the Notification's tab
				GLang.call(GLang.dr.resolveName("show_message_async"), [GLang.voidValue, GLang.stringValue(full)]);
				notification.close();
			}
		})
	}
